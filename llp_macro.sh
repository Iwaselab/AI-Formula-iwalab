...
tmux send-keys -t $SESSION "ros2 bag play -s mcap /home/fuga/AIFMovie/rosbag/0727_finalmpc --loop --topics /aiformula_sensing/zed_node/left_image/undistorted /aiformula_sensing/vehicle_info /aiformula_sensing/zed_node/imu" C-m 
...
tmux send-keys -t $SESSION:2 "export PYTHONPATH=~/workspace/ros2_ws_git/src/AI-Formula-iwalab/perception/yolop:\$PYTHONPATH" C-m
tmux send-keys -t $SESSION:2 "export PYTHONPATH=~/workspace/ros2_ws_git/src/AI-Formula-iwalab/perception/yolop/yolop:\$PYTHONPATH" C-m
tmux send-keys -t $SESSION:2 "ros2 launch object_road_detector_old object_road_detector_old.launch.py" C-m
...
tmux send-keys -t $SESSION:3 "ros2 launch detection_fusion detection_fusion.launch.py" C-m
...
tmux send-keys -t $SESSION:4 "ros2 launch sample_vehicle vehicle_tf_broadcaster.launch.py" C-m
...
tmux send-keys -t $SESSION:5 "ros2 launch lane_line_publisher lane_line_publisher.launch.py debug:=true" C-m
...
tmux send-keys -t $SESSION:6 "ros2 launch yolo_object_detector yolo_object_detector.launch.py" C-m
...
tmux send-keys -t $SESSION:7 "ros2 launch odometry_publisher gyro_odometry_publisher.launch.py" C-m
...
tmux send-keys -t $SESSION:8 "ros2 launch object_publisher object_publisher.launch.py" C-m
