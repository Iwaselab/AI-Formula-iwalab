...
tmux send-keys -t $SESSION "ros2 bag play -s mcap /home/fuga/AIFMovie/rosbag/all_out --loop --topics /aiformula_sensing/zed_node/left_image/undistorted /aiformula_sensing/vehicle_info /aiformula_sensing/zed_node/imu" C-m 
...
tmux send-keys -t $SESSION:2 "sleep 1 && ros2 launch object_road_detector object_road_detector.launch.py" C-m

# 3つ目のウィンドウ: object_road_detector(yolop使用)
tmux new-window -t $SESSION
...
tmux send-keys -t $SESSION:3 "sleep 1 && ros2 launch object_road_detector_old object_road_detector_old.launch.py" C-m

# 4つ目のウィンドウ: detection_fusion
tmux new-window -t $SESSION
...
tmux send-keys -t $SESSION:4 "sleep 1 && ros2 launch detection_fusion detection_fusion.launch.py" C-m

# 5つ目のウィンドウ: vehicle_tf_broadcaster
tmux new-window -t $SESSION
...
tmux send-keys -t $SESSION:5 "sleep 1 && ros2 launch sample_vehicle vehicle_tf_broadcaster.launch.py" C-m

# 6つ目のウィンドウ: lane_line_publisher
tmux new-window -t $SESSION
...
tmux send-keys -t $SESSION:6 "sleep 1 && ros2 launch lane_line_publisher lane_line_publisher.launch.py debug:=true" C-m

# 7つ目のウィンドウ: yolo_object_detector
tmux new-window -t $SESSION
...
tmux send-keys -t $SESSION:7 "sleep 1 && ros2 launch yolo_object_detector yolo_object_detector.launch.py" C-m

# 8つ目のウィンドウ: gyro_odometry_publisher
tmux new-window -t $SESSION
...
tmux send-keys -t $SESSION:8 "sleep 1 && ros2 launch odometry_publisher gyro_odometry_publisher.launch.py" C-m

# 9つ目のウィンドウ: object_publisher
tmux new-window -t $SESSION
...
tmux send-keys -t $SESSION:9 "sleep 1 && ros2 launch object_publisher object_publisher.launch.py" C-m
...
tmux send-keys -t $SESSION:10 "sleep 2 && ros2 launch extremum_seeking_mpc extremum_seeking_mpc.launch.py" C-m
...
tmux send-keys -t $SESSION:11 "sleep 2 && python3 twist_logger.py --topic /aiformula_control/extremum_seeking_mpc/cmd_vel --output twist_log.csv" C-m

# # 12つ目のウィンドウ: road_surface_segmenter
# tmux new-window -t $SESSION
# tmux send-keys -t $SESSION:12 "cd ~/workspace/ros2_ws_git" C-m
# tmux send-keys -t $SESSION:12 "source /opt/ros/foxy/setup.bash" C-m
# tmux send-keys -t $SESSION:12 "source install/setup.bash" C-m
# tmux send-keys -t $SESSION:12 "export PYTHONPATH=~/workspace/ros2_ws_git/src/AI-Formula-iwalab/perception/yolopv2:\$PYTHONPATH" C-m
# tmux send-keys -t $SESSION:12 "ros2 launch road_surface_segmenter road_surface_segmenter.launch.py" C-m
