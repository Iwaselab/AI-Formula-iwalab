...
class DetectionFusion(Node):
    ...
    def __init__(self) -> None:
        ...
        buffer_size = 10
        self._mask_pub = self.create_publisher(Image, 'pub_lane_mask', buffer_size)
        self._anno_pub = self.create_publisher(Image, 'pub_annotated_image', buffer_size)

        self.create_subscription(Image, 'sub_image', self._cache('_image'), buffer_size)
        self.create_subscription(Image, 'sub_lane_old', self._cache('_mask_old'), buffer_size)
        self.create_subscription(Image, 'sub_lane_new', self._cb_lane_new, buffer_size)

    def _cache(self, attr: str):
        encoding = 'bgr8' if attr == '_image' else 'mono8'
        def cb(msg: Image) -> None:
            try:
                setattr(self, attr, self.cv_bridge.imgmsg_to_cv2(msg, encoding))
    ...
    def _cb_lane_new(self, msg: Image) -> None:
        try:
            mask_new = self.cv_bridge.imgmsg_to_cv2(msg, 'mono8')
        except CvBridgeError as e:
